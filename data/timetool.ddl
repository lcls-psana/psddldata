  ...
  [[pack(4)]]
{
  ...
  /*  Size of projections */
  uint32_t signal_projection_size()
  [[language("C++")]] @{ return ((@self.write_projections()&&!@self.use_full_roi()) ?
  (@self.project_axis()==X ?
     @self.sig_roi_hi().column()-@self.sig_roi_lo().column()+1 :
     @self.sig_roi_hi().row   ()-@self.sig_roi_lo().row   ()+1) : 0); @}

  uint32_t sideband_projection_size()
  [[language("C++")]] @{ return ((@self.write_projections() &&
                                  @self.subtract_sideband() &&
                                  !@self.use_full_roi()) ?
  (@self.project_axis()==X ?
     @self.sb_roi_hi().column()-@self.sb_roi_lo().column()+1 :
     @self.sb_roi_hi().row   ()-@self.sb_roi_lo().row   ()+1) : 0); @}
  ...
  uint32_t signal_x_size()
  [[language("C++")]] @{ return ((@self.write_projections()&&@self.use_full_roi()) ?
  (@self.sig_roi_hi().column()-@self.sig_roi_lo().column()+1) : 0); @}

  uint32_t signal_y_size()
  [[language("C++")]] @{ return ((@self.write_projections()&&@self.use_full_roi()) ?
  (@self.sig_roi_hi().row   ()-@self.sig_roi_lo().row   ()+1) : 0); @}

  uint32_t signal_size()
  [[language("C++")]] @{ return @self.signal_x_size() * @self.signal_y_size(); @}

  uint32_t sideband_x_size()
  [[language("C++")]] @{ return ((@self.write_projections() &&
                                  @self.subtract_sideband() &&
                                  @self.use_full_roi()) ?
  (@self.sb_roi_hi().column()-@self.sb_roi_lo().column()+1) : 0); @}

  uint32_t sideband_y_size()
  [[language("C++")]] @{ return ((@self.write_projections() &&
                                  @self.subtract_sideband() &&
                                  @self.use_full_roi()) ?
  (@self.sb_roi_hi().row   ()-@self.sb_roi_lo().row   ()+1) : 0); @}
  ...
  [[language("C++")]] @{ return @self.sideband_x_size() * @self.sideband_y_size(); @}

  uint32_t reference_x_size()
  [[language("C++")]] @{ return ((@self.write_projections()&&@self.use_full_roi()) ?
  (@self.ref_roi_hi().column()-@self.ref_roi_lo().column()+1) : 0); @}

  uint32_t reference_y_size()
  [[language("C++")]] @{ return ((@self.write_projections()&&@self.use_full_roi()) ?
  (@self.ref_roi_hi().row   ()-@self.ref_roi_lo().row   ()+1) : 0); @}

  uint32_t reference_size()
  [[language("C++")]] @{ return @self.reference_x_size() * @self.reference_y_size(); @}
  ...
  [[config(ConfigV3)]]
{
  ...
  /*  Signal */
  int32_t _full_signal    [@config.signal_y_size()][@config.signal_x_size()] -> full_signal;

  /*  Sideband */
  int32_t _full_sideband  [@config.sideband_y_size()][@config.sideband_x_size()] -> full_sideband;

  /*  Reference */
  int32_t _full_reference [@config.reference_y_size()][@config.reference_x_size()] -> full_reference;
